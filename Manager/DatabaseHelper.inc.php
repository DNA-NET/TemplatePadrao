...
class DatabaseConn
{
	...
	public function connect() {
        ...
        if (! $link) {
            throw new DatabaseException(sprintf('Cannot connect to database. mysqli_connect() to %s with login %s fails', $this->host, $this->user)
            );
        }
    }

    public function selectDatabase() {
        $ret = @mysqli_select_db($this->db_name, $this->link);
        if (! $ret) {
            throw new DatabaseException("Cannot select database {$this->db_name}");
        }
    }
	...
	{
		$result = db_query($query, $this->link);
	...
	{
		return db_real_escape_string($str, $this->link);
	}
